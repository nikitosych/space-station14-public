...
comp-climbable-user-climbs-force = Вы заставляете { THE($moved-user) } взабраться на { THE($climbable) }!

# Shown to others when someone forces other $moved-user to climb on $climbable
comp-climbable-user-climbs-force-other = { CAPITALIZE(THE($user)) } заставляет { THE($moved-user) } взабраться на { THE($climbable) }!
...
comp-climbable-cant-climb = Вы не можете взабраться!

# Shown to you when your character tries to force someone else who can't climb onto a climbable
comp-climbable-target-cant-climb = { CAPITALIZE(THE($moved-user)) } не может взабраться туда!